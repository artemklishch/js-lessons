...
// const arb = new ArrayBuffer();
// console.log(Array.isArray(arb));
...
//   on(event, listener) {
//     this.events.push({ eventName: event, listener });
//   }
//   emit(event, options) {
//     const mainEventListener = this.events.find((e) => e.eventName === event);
//     if (mainEventListener) {
//       mainEventListener.listener(`{ message: ${options.message} }`);
//     }

//     const onceEventListener = this.onces.find((e) => e.eventName === event);
//     if (onceEventListener) {
//       onceEventListener.listener(`{ message: ${options.message} }`);
//       this.onces = this.onces.filter((o) => o.eventName !== event);
//     }
//   }
//   off(event, listener) {
//     this.events = this.events.filter((e) => e.eventName !== event);
//   }
//   once(event, cb) {
//     this.onces.push({ eventName: event, listener: cb });
//   }
// }
...
// // Output: Listener: { message: 'Hello World' }

// emitter.off("event1", listener);
// emitter.emit("event1", { message: "Hello again" });
// // No output, as the listener was removed.

// emitter.once("event2", (data) => console.log("Once:", data));
// emitter.emit("event2", { message: "This is once" });
// // Output: Once: { message: 'This is once' }

// emitter.emit("event2", { message: "This will not be seen" });
// // No output, as the listener has already been executed.
